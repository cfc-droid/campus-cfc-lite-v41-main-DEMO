/* ============================================================================
 🎧 CFC-VOICE READER PRO — V26 REAL FINAL
 ✔ Oración completa punto → punto (multilínea, sin importar saltos, etiquetas, espacios)
 ✔ Highlight INLINE seguro (No rompe HTML)
 ✔ Android + PC (boundary + fallback)
 ✔ Resume exacto
 ✔ Sin alterar nada del Campus
============================================================================ */
...
/* =========================
   LOG
========================= */
const log = (...m) => console.log("🎧 V26:", ...m);

/* ============================================================================
   AUDIO UNLOCK (Android)
============================================================================ */
...
/* ============================================================================
   NORMALIZAR TEXTO (tolerante)
============================================================================ */
function normalizeForRegex(str){
    return str
        .replace(/\s+/g," ")
        .replace(/[.*+?^${}()|[\]\\]/g, "\\$&")
        .trim();
...
/* ============================================================================
   RESALTAR ORACIÓN (MULTILÍNEA REAL)
============================================================================ */
function highlightSentenceHTML(sentence) {
    clearMarks();
    if (!sentence) return;
    ...
    const norm = normalizeForRegex(sentence);

    // tolera espacios, saltos, etiquetas, etc.
    const flexible = new RegExp(norm.replace(/ /g, "[\\s\\S]{0,40}?"), "i");

    const match = html.match(flexible);
    if (!match) return;

    const replaced = `<mark class="cfcInlineHL">${match[0]}</mark>`;
    html = html.replace(flexible, replaced);

    container.innerHTML = html;
    activeMarks = Array.from(document.querySelectorAll(".cfcInlineHL"));
}
...
/* ============================================================================
   OBTENER ORACIÓN COMPLETA (PUNTO → PUNTO)
============================================================================ */
...
function highlightFallback(sentence){
    ...
    highlightSentenceHTML(sentence);

    const ms = Math.max(1500, sentence.split(" ").length * (350 / rate));
    highlightTimer = setTimeout(() => clearMarks(), ms);
}
...
function openPanel(){
    ...
    panel.innerHTML = `
        <h4 style="color:#FFD700;margin:0 0 6px;font-size:14px;">Narrador IA</h4>
        ...
        <style>
            .cfcBtn{width:100%;padding:6px;background:#FFD700;color:#000;font-weight:700;
    ...
    panel.querySelector("#ttsClose").onclick = ()=>{ stopReading(); panel.remove(); };
}
